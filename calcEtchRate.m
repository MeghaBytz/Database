function etchRate = calcEtchRate(Te,ionFlux,radicalFlux,current)
global noUnknowns
global numberOfIons
...
dim = 2; %2 dimensions because we are calculating Vx and Vz
...
for j = 1:dim
    ...
    for i = 1: numberOfIons
        desorption = ionStimulatedDesorption(i)*ionFlux(j,i) + desorption;
    end

    for i = 1:numberOfRadicals
        chemicalEtchTerm = rxnProb(i)*SCl(i)*radicalFlux(j,i) + chemicalEtchTerm;
    end

    surfaceCoverage = chemicalEtchTerm/(desorption+chemicalEtchTerm);

    for i = 1:numberOfRadicals
        chemicalEtch = (1-surfaceCoverage)*rxnProb(i)*radicalFlux(j,i) + chemicalEtch;
    end

    for i = 1:numberOfRadicals
        sputtering = (1-surfaceCoverage)*sputteringYield(i)*ionFlux(j,i) + sputtering;
    end
    %EtchRate = (1-RxnProb*FluxCl/(RxnProb*FluxCl + Yd*TotalPosFlux))*(RxnProb*FluxCl+Ys*FluxAr_pos);
    etchRate(j) = chemicalEtch + sputtering;
    etchRate(j) = etchRate(j)*6e+11*M/(Na*density)+current(end)*randn; %nm/min
